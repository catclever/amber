module Amber
  class Engine
    module Planner
      ...
      def spawn_dynamic_job_for_missing_requirement(goal)
        queue = @context.get(:__amber_dynamic_jobs) || []
        job_sym = :"auto_solve_#{Time.now.to_i}_#{rand(1000)}"
        
        queue << {
          name: job_sym.to_s,
          objective: "Resolve missing Context Requirement: #{goal}. Break this down using your spawn_job tool, or solve it yourself and write to context.",
          condition: nil
        }
